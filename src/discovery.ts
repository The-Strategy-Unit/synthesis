...
async function crossSourceCandidates(
  ...
  for (const seedId of seeds) {
    ...
    const embedding = db.getEmbedding(seedId);
    if (embedding) {
      let added = 0;
      for (const nearest of db.findNearest(seedId, embedding, nearestLimit)) {
    ...
    for (
      const match of db.findIntegrationCandidates(
        `${seed.title}\n${seed.body}`,
        16,
      )
    ) {
...
function discoveryCoverage(
  ...
  const stored = db.getDiscoveryCandidateCoverage(generation);
  return {
    generation,
    eligiblePages,
...
export async function generateDiscoveries(
  ...
  const queuedRecords = db.getDiscoveryCandidates(
    generation,
    "queued",
    limit,
  );
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    db.withTransaction(() => {
      for (
        ...
        const insertedId = db.addDiscovery({
          fingerprint,
        ...
        const discoveryId = insertedId ??
          db.getDiscoveries().find((item) => item.fingerprint === fingerprint)
            ?.id;
