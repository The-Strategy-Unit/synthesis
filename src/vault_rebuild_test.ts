...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const staleSourceId = db.addSource(
        "stale-source",
        "Stale source",
        null,
        "text",
      ...
      const staleNoteId = db.addNote(
        "Stale note",
        `${dir}/stale.md`,
        null,
        "text",
      );
      db.indexNote(staleNoteId, "Stale note", "Stale searchable body.");
      db.attachNoteSource(staleNoteId, staleSourceId, "new");
      db.addIngestProposal(staleSourceId, '{"version":1,"changes":[]}');
      globalThis.fetch = () => {
      ...
      const alpha = db.getNoteByExactTitle("Alpha mechanism");
      const beta = db.getNoteByExactTitle("Beta observation");
      assert.ok(alpha);
      assert.ok(beta);
      assert.deepEqual(
        db.searchKeyword("searchable").map((item) => item.id),
        [alpha.id],
      );
      const localSource = db.getSourceByHash(localHash);
      assert.ok(localSource);
      assert.equal(db.getNotesForSource(localSource.id)[0].id, beta.id);
      assert.equal(db.getNotesForSource(localSource.id)[0].action, "reference");
      ...
      assert.equal(db.getNoteByExactTitle("Alpha mechanism")?.id, alpha.id);
    } finally {
