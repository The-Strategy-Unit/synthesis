...
import { notesDir } from "../app/config.ts";
...
export async function saveWikiSynthesis(
  ...
  const existing = db.notes.getNoteByExactTitle(page.title);
  ...
  const sources = db.sources.getSourcesForNotes(citedNoteIds);
  if (sources.length === 0) {
  ...
  try {
    noteId = db.withTransaction(() => {
      const id = db.notes.addNote(page.title, filePath, null, "query");
      db.notes.indexNote(id, page.title, page.body);
      db.search.upsertEmbedding(id, embedding);
      for (const source of sources) {
        db.sources.attachNoteSource(id, source.id, "query");
      }
      return id;
    });
  } catch (error) {
  ...
  if (db.search.semanticIndexStatus().complete) {
    db.search.computeLinksFor([noteId]);
