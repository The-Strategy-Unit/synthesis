...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      db.notes.addNote("orphan finding", duplicatePath, null, "text");
      db.notes.addNote("Unreadable page", `${dir}/missing.md`, null, "text");

      db.sources.attachNoteSource(treatment, sourceId, "contradict");
      db.sources.attachNoteSource(confidence, sourceId, "new");
