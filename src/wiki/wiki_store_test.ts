...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const citedId = db.notes.addNote(
        "Treatment effect",
      ...
      db.notes.indexNote(citedId, "Treatment effect", "The evidence is mixed.");
      db.sources.attachNoteSource(citedId, sourceId, "new");
      ...
      const stored = db.notes.getNote(saved.id);
      assert.ok(stored);
      ...
      assert.deepEqual(
        db.sources.getNotesForSource(sourceId).map((note) => ({
          title: note.title,
          action: note.action,
        })),
        [
      ...
      assert.ok(db.search.getEmbedding(saved.id));
      assert.ok(
        db.search.findIntegrationCandidates("available evidence mixed", 10)
          .some(
