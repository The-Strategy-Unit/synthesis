...
import { findClaimCitations, parseWikiPage, renderWikiPage } from "./wiki.ts";
  ...
  fn: async () => {
    ...
    try {
      await withReviewVault(async (db, dir) => {
        ...
        assert.equal(
          db.getIngestProposal(staged.proposal.id)?.status,
          "approved",
        );
        assert.match(
  ...
  fn: async () => {
    ...
    try {
      await withReviewVault(async (db) => {
        ...
        assert.equal(applied.newCount, 1);
        assert.deepEqual(
          db.getAllNotes().map((note) => note.title),
          ["Second candidate"],
        );
        const note = db.getAllNotes()[0];
        ...
        const source = db.getAllSources()[0];
        ...
        assert.deepEqual(
          db.getNotesForSource(source.id).map(({ title, action }) => ({
            title,
            action,
          })),
  ...
  fn: async () => {
    ...
    try {
      await withReviewVault(async (db, dir) => {
        ...
        const noteId = db.addNote("Existing concept", notePath, null, "text");
        db.indexNote(noteId, "Existing concept", "Original durable knowledge.");
  ...
  fn: async () => {
    ...
    try {
      await withReviewVault(async (db, dir) => {
        ...
        const targetId = db.addNote("Target", targetPath, null, "text");
        db.indexNote(targetId, "Target", "Existing target knowledge.");
        ...
        db.addNote("Target", duplicatePath, null, "text");
        await assert.rejects(
          approveIngestProposal(db, staged.proposal.id, () => {}),
          InvalidWikiLinkError,
        );
