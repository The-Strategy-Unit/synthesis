...
class FailingAttachDb extends DB {
  ...
  constructor(dbPath: string) {
    super(dbPath);
    const attachNoteSource = this.sources.attachNoteSource.bind(this.sources);
    this.sources.attachNoteSource = (noteId, sourceId, action) => {
      if (this.failAttach) {
        throw new Error("forced provenance attachment failure");
      }
      attachNoteSource(noteId, sourceId, action);
    };
  }
}
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const notesAfterFirst = db.notes.getAllNotes();
      ...
      const firstSourceRecord = db.sources.getSourceByHash(firstHash);
      assert.ok(firstSourceRecord);
      assert.deepEqual(
        db.sources.getNotesForSource(firstSourceRecord.id).map((note) =>
          note.action
        ),
        ["new", "new"],
      );

      const target = db.notes.getNote(first.notes[0].id);
      const unaffected = db.notes.getNote(first.notes[1].id);
      assert.ok(target);
      ...
      const secondSourceRecord = db.sources.getSourceByHash(secondHash);
      assert.ok(secondSourceRecord);
      assert.deepEqual(
        db.sources.getNotesForSource(secondSourceRecord.id).map((note) => ({
          id: note.id,
          action: note.action,
        })),
      ...
      const embeddingBeforeFailure = db.search.getEmbedding(target.id);
      assert.ok(embeddingBeforeFailure);
      const indexedBeforeFailure = db.search.findIntegrationCandidates(
        "verified detail",
        10,
      ).find((candidate) => candidate.id === target.id);
      assert.ok(indexedBeforeFailure);
      const firstAttachmentsBefore = db.sources.getNotesForSource(
        firstSourceRecord.id,
      )
        .map((note) => ({ id: note.id, action: note.action }));
      const secondAttachmentsBefore = db.sources.getNotesForSource(
      ...
      assert.deepEqual(
        db.search.getEmbedding(target.id),
        embeddingBeforeFailure,
      );
      const indexedAfterFailure = db.search.findIntegrationCandidates(
        "verified detail",
        10,
      ).find((candidate) => candidate.id === target.id);
      ...
      assert.deepEqual(
        db.search.findIntegrationCandidates("failed update", 10),
        [],
      );
      assert.deepEqual(
        db.sources.getNotesForSource(firstSourceRecord.id).map((note) => ({
      ...
      assert.deepEqual(db.sources.getNotesForSource(thirdSourceRecord.id), []);

      const activeDb = db;
      const assertWikiUnchanged = async () => {
        assert.equal(await Deno.readTextFile(target.file_path), mergedMarkdown);
        assert.deepEqual(
          activeDb.search.getEmbedding(target.id),
          embeddingBeforeFailure,
        );
        const indexed = activeDb.search.findIntegrationCandidates(
          "verified detail",
          10,
        )
          .find((candidate) => candidate.id === target.id);
      ...
      const renamedSourceRecord = db.sources.getSourceByHash(
        await sha256(renamedSource.transcript),
      );
      assert.ok(renamedSourceRecord);
      assert.deepEqual(
        db.sources.getNotesForSource(renamedSourceRecord.id),
        [],
      );
      ...
      await assertWikiUnchanged();
      const malformedSourceRecord = db.sources.getSourceByHash(
        await sha256(malformedSource.transcript),
      );
      assert.ok(malformedSourceRecord);
      assert.deepEqual(
        db.sources.getNotesForSource(malformedSourceRecord.id),
        [],
      );
    } finally {
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const alpha = db.notes.getNote(targetIds.alpha);
      const beta = db.notes.getNote(targetIds.beta);
      assert.ok(alpha);
      assert.ok(beta);
      const alphaBefore = await Deno.readTextFile(alpha.file_path);
      const betaBefore = await Deno.readTextFile(beta.file_path);
      const alphaEmbeddingBefore = db.search.getEmbedding(alpha.id);
      const betaEmbeddingBefore = db.search.getEmbedding(beta.id);
      ...
      assert.deepEqual(db.search.getEmbedding(alpha.id), alphaEmbeddingBefore);
      assert.deepEqual(db.search.getEmbedding(beta.id), betaEmbeddingBefore);
      assert.equal(
      ...
      assert.deepEqual(
        db.search.findIntegrationCandidates("rewritten in memory", 10),
        [],
      );
      const failedSourceRecord = db.sources.getSourceByHash(
        await sha256(failingSource.transcript),
      );
      assert.ok(failedSourceRecord);
      assert.deepEqual(db.sources.getNotesForSource(failedSourceRecord.id), []);
    } finally {
      globalThis.fetch = originalFetch;
      config.vaultDir = originalVaultDir;
      db.close();
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const existingSourceId = db.sources.addSource(
        existingHash,
        "Existing source",
        null,
        "text",
      ...
      db.sources.attachNoteSource(existingId, existingSourceId, "new");
