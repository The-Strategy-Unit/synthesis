...
import { updateWikiCatalog } from "../wiki/wiki_store.ts";
...
function sourceReferencesForRewrite(
  ...
  for (const source of db.sources.getSourceProvenanceForNote(noteId)) {
    add({
      title: source.title,
...
async function prepareSingleSourceChanges(
  ...
  const knownSource = db.sources.getSourceByHash(contentHash);
  if (knownSource) {
  ...
  for (const note of distilled.notes) {
    for (
      const candidate of db.search.findIntegrationCandidates(
        `${note.title}\n${note.body}`,
        8,
      )
    ) {
  ...
  for (let i = 0; i < resolvedNotes.length; i++) {
    ...
    if (
      ...
      const existing = db.notes.getNote(decision.existing_id!);
      if (!existing) {
        throw new Error(
...
export function listIngestProposalReviews(
  ...
  return db.proposals.getIngestProposals(status).map((record) =>
...
export async function stageSingleSource(
  ...
  const knownSource = db.sources.getSourceByHash(hash);
  if (knownSource) {
    const applied = await appliedSourceResult(
      db,
      ingested,
    ...
    if (applied) return applied;
    const knownProposal = db.proposals.getIngestProposalForSource(
      knownSource.id,
    );
    if (knownProposal) {
...
export async function approveIngestProposal(
  ...
  for (const change of selectedChanges) {
    if (change.action === "new") {
      if (db.notes.getNoteByExactTitle(change.page.title)) {
    ...
    const existing = db.notes.getNote(change.pageId!);
    if (!existing) {
      throw new StaleIngestProposalError(
  ...
  const applied = await applyPreparedWikiChanges(
    ...
    providers,
    {
      ...
      signal,
      finalizeTransaction: () => {
        if (!db.proposals.reviewIngestProposal(id, "approved")) {
          throw new IngestProposalStateError(
...
export function rejectIngestProposal(
  ...
  if (!db.proposals.reviewIngestProposal(id, "rejected")) {
    throw new IngestProposalStateError(
      `Ingest proposal ${id} is already ${record.status}`,
    );
  }
